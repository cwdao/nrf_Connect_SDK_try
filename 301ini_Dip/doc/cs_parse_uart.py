...
def main() -> int:
    ...
    parser.add_argument("--csv", metavar="FILE", help="Append rows to CSV")
    args = parser.parse_args()

    reader = FrameReader()
    csv_file = open(args.csv, "a", encoding="utf-8") if args.csv else None
    if csv_file and csv_file.tell() == 0:
        csv_file.write("procedure_counter,type,ch,i_local,q_local,i_remote,q_remote\n")
        ...
        try:
            while True:
                ...
                for raw in reader.feed(chunk):
                    ...
                    if chs:
                        print(
                            f"[{n}] {tname} pc={pc} ap={info['ap']} "
                            f"N={info['channel_count']} ch=[{chs[0]}..{chs[-1]}]"
                        )
                    else:
                        print(f"[{n}] {tname} pc={pc} ap={info['ap']} N=0")
                    if args.dump_hex:
                        print(raw.hex())
                    if csv_file:
                        for ch, vals in info["iq"].items():
                            if ftype == TYPE_DIP_LOCAL:
                                ...
                                csv_file.write(f"{pc},{ftype},{ch},{i},{q},,\n")
                            else:
                                il, ql, ir, qr = vals  # type: ignore
                                csv_file.write(f"{pc},{ftype},{ch},{il},{ql},{ir},{qr}\n")
                        csv_file.flush()
