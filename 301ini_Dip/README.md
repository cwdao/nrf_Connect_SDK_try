...
| `DIP_REPORT_LOG_VERBOSE` | **1** | 多信道 `ch(i,q)` 分块打印；改为 **0** 则每报告仅 `DIP ok pc=…` |
...
3. **日志详略**（`main.c`，**可选值**；当前工程默认见上表）：

   ```c
   #define DIP_REPORT_LOG_VERBOSE 0   /* 每报告一行：DIP ok pc=… */
   #define DIP_REPORT_LOG_VERBOSE 1   /* 多信道本地 IQ 分块打印 ch(i,q) */
   ```
...
| DIP | `subevent_result_dip_cb` → 拷贝 `step_data_buf` → `bt_le_cs_step_data_parse` → mode 2/3 的 **PCT** → `bt_le_cs_parse_pct()` → 聚合到 `dip_parse_work_ctx`，再按宏打印 |
...
2. **Abort**：若 `subevent_done_status` 为 **aborted**，会打印 `pc`、`subevent_abort_reason`、`num_steps_reported`、`abort_step`、`procedure_done_status` 等，不解析 IQ。  
3. **IQ 与天线索引**：与 Nordic `cs_de.c::extract_pcts` 一致——`bt_le_cs_get_antenna_path` + `tone_info[antenna_path]`；**信道格点** `fft_ch = step->channel - 2`（`DIP_CS_CHANNEL_INDEX_OFFSET`），宽度 `DIP_MAX_FFT_CHANNELS`（与 cs_de 通道格点思路对齐）。  
4. **配置**：`CONFIG_BT_RAS_MAX_ANTENNA_PATHS`、`header.num_antenna_paths` 决定 `n_ap`；`LOCAL_PROCEDURE_MEM` / reassembly 相关配置限制单包 `step_data` 最大长度，超限则丢弃并 `LOG_ERR`。
...
| DIP 解析与输出 | `dip_parse_local_iq_from_subevent`、`dip_local_step_iq_cb`、`dip_print_local_report_multichannel` |
| DIP / RAS 宏 | `APP_CS_DIP_BYPASS_RAS`、`DIP_REPORT_LOG_VERBOSE` |
...
本项目 **仅维护本 `README.md`** 作为目录说明；历史单独的 DIP 说明已并入上文。
