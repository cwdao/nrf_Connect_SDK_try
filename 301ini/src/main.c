...
static void ranging_data_get_complete_cb(struct bt_conn *conn,
  ...
  ranging_count++;
  LOG_DBG("Ranging test: %d/%d (ranging_counter: %d)", 
          ranging_count, TEST_RANGING_COUNT, ranging_counter);
#endif
  ...
  if (err) {
    ...
    LOG_INF("Ranging error: %d/%d attempts, %d errors, %d success", 
            ranging_count, TEST_RANGING_COUNT, ranging_error_count, ranging_success_count);
    
    // 检查是否达到测试次数（即使失败也要检查）
    if (ranging_count >= TEST_RANGING_COUNT) {
      LOG_INF("Test completed! %d attempts (%d success, %d errors), disabling CS",
...
#ifdef TEST_RANGING_ENABLED
  if (ranging_count >= TEST_RANGING_COUNT) {
    LOG_INF("Test completed! %d attempts (%d success, %d errors, %d overwritten), disabling CS",
            TEST_RANGING_COUNT, ranging_success_count, ranging_error_count, ranging_overwritten_count);
...
static void ranging_data_overwritten_cb(struct bt_conn *conn,
  ...
  LOG_INF("OW! %i (total overwritten: %d)", ranging_counter, ranging_overwritten_count);
  
  // 如果覆盖频率过高，给出警告
  if (ranging_overwritten_count > 0 && ranging_count > 0) {
    float overwrite_rate = (float)ranging_overwritten_count / ranging_count * 100.0f;
    if (overwrite_rate > 20.0f) {  // 覆盖率超过20%
      LOG_WRN("High overwrite rate: %.1f%% (%d/%d) - Consider reducing ranging frequency!", 
              (double)overwrite_rate, ranging_overwritten_count, ranging_count);
    }
  }
#else
  LOG_INF("OW! %i", ranging_counter);
#endif
}
...
static void button0_work_handler(struct k_work *work) {
  ...
  LOG_INF("Button 0 pressed: Starting test with %d ranging measurements",
          TEST_RANGING_COUNT);
  LOG_INF("Counters reset - attempts: %d, success: %d, errors: %d, overwritten: %d",
          ranging_count, ranging_success_count, ranging_error_count, ranging_overwritten_count);
#endif
  params.enable = 1;
  bt_le_cs_procedure_enable(connection, &params);
